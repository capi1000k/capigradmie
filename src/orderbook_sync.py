...
Stage 1.1A additions:
  - receive_ts  : timestamp when raw bytes arrived from WS
  - exchange_ts : Binance event timestamp (field "E" in depth stream)
  - process_ts  : timestamp when microstructure dict was fully computed
  - latency_ms  : receive_ts - exchange_ts  (network latency in ms)
...
class OrderBookSyncer:
    ...
    def __init__(self, shared_state: dict, wal: WALBuffer):
        ...
        self._latency = LatencyMonitor("orderbook")  # ← YANGI
        # DQ monitor — shared instance via shared_state
        self._dq = None  # lazily set in run()
    ...
    def _compute_microstructure(
        self,
        receive_ts: pd.Timestamp,
        exchange_ts: pd.Timestamp,
    ) -> dict | None:
        """
        Compute microstructure metrics from current book state.

        Parameters
        ----------
        receive_ts  : pd.Timestamp
            UTC timestamp captured immediately after ws.recv() — before json.loads().
        exchange_ts : pd.Timestamp
            Binance event timestamp from field "E" in the depth stream payload.

        Returns
        -------
        dict | None
            Microstructure snapshot, or None if book is empty.
        """
        ...
        # ── Stage 1.1A: latency telemetry ────────────────────────────────────
        # process_ts = funksiya o'z ishini tugatgan lahza
        # latency_ms = receive_ts - exchange_ts  (network latency)
        process_ts = utc_now()
        latency_ms = (receive_ts - exchange_ts).total_seconds() * 1000

        return {
            "timestamp":   exchange_ts,    # primary key — Binance event time
            "best_bid":    best_bid,
            "best_ask":    best_ask,
            "spread":      spread,
            "mid_price":   mid_price,
            ...
            # ── latency fields ───────────────────────────────────────────────
            "exchange_ts": exchange_ts,    # Binance server time
            "receive_ts":  receive_ts,     # bytes arrived at our socket
            "process_ts":  process_ts,     # computation finished
            "latency_ms":  latency_ms,     # network latency
        }
    ...
    async def run(self) -> None:
        ...
        from src.data_quality import DataQualityMonitor
        self._dq = self.shared_state.setdefault("_dq_monitor", DataQualityMonitor())
            ...
            try:
                async with aiohttp.ClientSession() as http_session:
                    ...
                    async with websockets.connect(
                        ...
                        async for raw in ws:
                            # ── Stage 1.1A: receive_ts MUST be first ─────────
                            # json.loads() dan OLDIN — sof network arrival vaqti
                            receive_ts = utc_now()
                            event = json.loads(raw)

                            # exchange_ts: Binance depth stream "E" field (ms)
                            exchange_ts = to_timestamp(int(event["E"]))
                            ...
                            if not ok:
                                self._dq.record_gap()
                                self._dq.record_resync()
                                # gap — rebuild
                                await self._fetch_snapshot(http_session)
                                continue

                            snap = self._compute_microstructure(receive_ts, exchange_ts)
                            if snap:
