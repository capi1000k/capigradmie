...
app.layout = dbc.Container(
    ...
    children=[
        ...
        dbc.Row([
         _card("Lat P50 ms", "stat-lat-p50", BLUE),
         _card("Lat P95 ms", "stat-lat-p95", ORANGE),
         _card("Lat P99 ms", "stat-lat-p99", RED),
         _card("Lat Avg ms", "stat-lat-avg", TEXT),
     ], className="mb-2 g-2"),
        dbc.Row([
         _card("Regime", "stat-regime", VIOLET),
     ], className="mb-2 g-2"),

        dbc.Row([
            _card("DQ Score",      "stat-dq-score",  GREEN),
            _card("Trade Fresh s", "stat-dq-tfresh", TEXT),
            _card("OB Fresh s",    "stat-dq-ofresh", TEXT),
            _card("Gaps",          "stat-dq-gaps",   ORANGE),
            _card("Msg Rate t/s",  "stat-dq-rate",   BLUE),
        ], className="mb-2 g-2"),
...
@app.callback(
    ...
    Output("stat-db",     "children"),
        Output("stat-regime", "children"),
    Output("stat-lat-p50", "children"),
    Output("stat-lat-p95", "children"),
    Output("stat-lat-p99", "children"),
    Output("stat-lat-avg", "children"),
    Output("stat-dq-score",  "children"),
    Output("stat-dq-tfresh", "children"),
    Output("stat-dq-ofresh", "children"),
    Output("stat-dq-gaps",   "children"),
    Output("stat-dq-rate",   "children"),
    # ── trigger ────────────────────────────────────────────────────────────────
...
def update(_n):
    ...
    try:
        ...
        lat_p50_s = lat_p95_s = lat_p99_s = lat_avg_s = "—"
        try:
            lat = _shared_state.get("latency_stats") or {}
            if lat and lat.get("count", 0) > 0:
                lat_p50_s = f"{lat['p50']:.1f}"
                lat_p95_s = f"{lat['p95']:.1f}"
                lat_p99_s = f"{lat['p99']:.1f}"
                lat_avg_s = f"{lat['avg']:.1f}"
        except Exception:
            log.warning("[dash] latency stats:\n%s", traceback.format_exc())

        regime      = _shared_state.get("regime") or {}
        regime_label = regime.get("label", "—")
        regime_conf  = regime.get("confidence", 0.0)
        regime_s     = f"{regime_label}  {regime_conf:.2f}" if regime_label != "—" else "—"

        dq_score_s = dq_tfresh_s = dq_ofresh_s = dq_gaps_s = dq_rate_s = "—"
        try:
            dq = _shared_state.get("dq") or {}
            if dq:
                sc = dq.get("score", 0)
                dq_score_s  = f"{sc:.0f}"
                dq_tfresh_s = f"{dq.get('trade_fresh_s', 0):.1f}"
                dq_ofresh_s = f"{dq.get('ob_fresh_s', 0):.1f}"
                dq_gaps_s   = str(dq.get("gap_count", 0))
                dq_rate_s   = f"{dq.get('trades_per_s', 0):.1f}"
        except Exception:
            log.warning("[dash] dq stats:\n%s", traceback.format_exc())
        ...
        return (
            ...
            regime_s, lat_p50_s, lat_p95_s, lat_p99_s, lat_avg_s,
            dq_score_s, dq_tfresh_s, dq_ofresh_s, dq_gaps_s, dq_rate_s,
        )
