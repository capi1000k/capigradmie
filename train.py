...
Lopez de Prado metodologiyasi:
    ...
    2. PURGE — train/val chegarasida label horizon qadar
               barlar o'chiriladi (label overlap yo'q)
    3. EMBARGO — val dan keyin ham horizon qadar barlar
                 keyingi fold train ga kirmaydi

Bu lookahead bias ni butunlay yo'q qiladi.

Embargo:
    max_bars_max = 16  →  har fold orasida 16 bar o'tkazib yuboriladi
    Bu Triple Barrier label horizon bilan mos.
...
N_FOLDS      = 5
TEST_RATIO   = 0.15        # oxirgi 15% — hech qachon train da emas
EMBARGO_BARS = 16          # max_bars_max bilan mos (label horizon)
...
def purged_walk_forward_splits(n: int,
                                n_folds: int = 5,
    ...
    Har fold orasida `embargo` bar o'tkazib yuboriladi.

    Vizual:
        Fold 1: [=====TRAIN=====][emb][==VAL==][emb]
        Fold 2: [=======TRAIN=======][emb][==VAL==][emb]
        Fold 3: [=========TRAIN=========][emb][==VAL==][emb]
        ...
        TEST:   [                              ][emb][====TEST====]

    embargo = label horizon (max_bars_max = 16)
    """
    test_size  = int(n * test_ratio)
    train_end  = n - test_size - embargo   # test oldidan ham embargo
...
def train_fold(X_train, y_train,
    ...
    # Val da signal precision
    max_p     = val_proba.max(axis=1)
    sig_mask  = (max_p >= 0.55) & (val_pred != 1)
    if sig_mask.sum() > 0:
        sig_prec = (val_pred[sig_mask] == y_val.values[sig_mask]).mean()
    else:
        sig_prec = 0.0

    print(f"  Fold {fold_num}: "
          f"iter={model.best_iteration:>4d}  "
          f"val_acc={val_acc:.4f}  "
          f"sig_prec@0.55={sig_prec:.4f}  "
...
def tune_threshold(oof_proba: np.ndarray,
    ...
    print(f"  {'Thresh':>7}  {'Signal%':>8}  {'Precision':>10}  "
          f"{'Recall':>8}  {'F1':>6}")
    ...
    for thr in thresholds:
        mask      = max_proba >= thr
        pred_filt = np.where(mask, pred_class, 1)

        sig_pred = pred_filt[mask & (pred_class != 1)]
...
def evaluate(y_true, proba, threshold, label=""):
    ...
    print(classification_report(y_true, pred_raw,
...
def show_feature_importance(models, feature_names, top_n=25):
    ...
    for rank, i in enumerate(idx, 1):
        ...
        print(f"  {rank:>3}. {feature_names[i]:<35} "
              f"{importances[i]:>10.1f}  {bar}")
...
def train():
    ...
    # OOF baholash
    evaluate(oof_true, oof_proba, threshold, label="OOF (CV)")
    ...
    print(f"  ✅ Final model tayyor  ({len(X_trainval):,} bar bilan)")
